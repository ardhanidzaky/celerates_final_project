...
def main():
    ...
    spark = SparkSession.builder \
        .appName("Create Category Data Mart") \
        .getOrCreate()
    transactional_df = spark.createDataFrame(df)

    # Perform transformation
    data_mart1_df = transactional_df \
        .groupBy(date_format(col("date"), "yyyy-MM-dd").alias("date"), "category") \
        .agg(count("*").alias("request_count"))
    
    # Convert Spark DataFrame to Pandas DataFrame
    result_df = data_mart1_df.toPandas()
    ...
    for _, row in result_df.iterrows():
        insert_query = """
            INSERT INTO category_pd (date, category, request_count)
            VALUES (%s, %s, %s);
        """
        dest_cursor.execute(insert_query, (row['date'], row['category'], row['request_count']))
